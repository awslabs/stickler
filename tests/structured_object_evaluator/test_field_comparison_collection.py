...
class PrimitiveListModel(StructuredModel):
    ...
    tags: List[str] = ComparableField(comparator=LevenshteinComparator(), weight=1.0)
    scores: List[float] = ComparableField(comparator=NumericComparator(), weight=1.0)
...
class TestFieldComparisonCollection:
    ...
    def test_basic_field_comparisons_exact_matches(self):
        ...
        name_comparison = next(
            fc for fc in field_comparisons if fc["expected_key"] == "name"
        )
    ...
    def test_basic_field_comparisons_partial_matches(self):
        ...
        model2 = SimpleModel(
            name="Jon Doe", age=30, active=True, score=95.01
        )  # Slight differences
        ...
        score_comparison = next(
            fc for fc in field_comparisons if fc["expected_key"] == "score"
        )
    ...
    def test_basic_field_comparisons_non_matches(self):
        ...
        model1 = SimpleModel(name="John Doe", age=30, active=True, score=95.0)
        model2 = SimpleModel(
            name="Jane Smith", age=25, active=False, score=50.0
        )  # Different values
        ...
        active_comparison = next(
            fc for fc in field_comparisons if fc["expected_key"] == "active"
        )
        assert active_comparison["match"] is False
        assert (
            active_comparison["score"] == 0.0
        )  # ExactComparator gives 0 for different booleans
    ...
    def test_nested_structured_model_field_comparisons(self):
        ...
        street_comparison = next(
            fc for fc in field_comparisons if fc["expected_key"] == "address.street"
        )
    ...
    def test_nested_field_comparisons_partial_matches(self):
        ...
        person2 = Person(
            ...
            age=30,
            address=Address(
                street="123 Main Street", city="NYC", zipcode="10002"
            ),  # Differences
        )
        ...
        city_comparison = next(
            fc for fc in field_comparisons if fc["expected_key"] == "address.city"
        )
        assert city_comparison["match"] is False  # Below 0.8 threshold

        # Zipcode should be non-match (ExactComparator)
        zipcode_comparison = next(
            fc for fc in field_comparisons if fc["expected_key"] == "address.zipcode"
        )
    ...
    def test_list_field_comparisons_hungarian_matching(self):
        ...
        widget_a_comparisons = [
            fc for fc in list_comparisons if fc["expected_value"] == "Widget A"
        ]
        ...
        widget_b_comparisons = [
            fc for fc in list_comparisons if fc["expected_value"] == "Widget B"
        ]
    ...
    def test_list_field_comparisons_partial_matches(self):
        ...
        invoice2 = Invoice(
            ...
            line_items=[
                LineItem(
                    product="Widget Alpha", quantity=2, price=10.5
                ),  # Similar product, different price
                LineItem(
                    product="Gadget C", quantity=3, price=15.0
                ),  # Different product
            ],
            total=41.0,
        )
        ...
        product_comparisons = [
            fc for fc in list_comparisons if fc["expected_key"].endswith(".product")
        ]
        widget_a_comparison = next(
            (fc for fc in product_comparisons if fc["expected_value"] == "Widget A"),
            None,
        )
        ...
        if widget_a_comparison:
            ...
            assert (
                widget_a_comparison["match"] is False
            )  # Should be below 0.8 threshold
    ...
    def test_primitive_list_field_comparisons(self):
        ...
        model1 = PrimitiveListModel(
            tags=["python", "testing", "stickler"], scores=[95.5, 87.2, 92.1]
        )
        ...
        tag_comparisons = [
            fc for fc in field_comparisons if fc["expected_key"].startswith("tags[")
        ]
        score_comparisons = [
    ...
    def test_edge_case_mismatched_list_lengths(self):
        ...
        model1 = PrimitiveListModel(
            tags=["tag1", "tag2", "tag3"], scores=[95.0, 87.0, 92.0]
        )
        model2 = PrimitiveListModel(
    ...
    def test_integration_with_confusion_matrix(self):
        ...
        result = model1.compare_with(
            model2, document_field_comparisons=True, include_confusion_matrix=True
        )
    ...
    def test_integration_with_document_non_matches(self):
        ...
        result = model1.compare_with(
            model2, document_field_comparisons=True, document_non_matches=True
        )
