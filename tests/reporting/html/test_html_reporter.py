...
class TestEvaluationHTMLReporter:
    ...
    def test_generate_report_success_individual_results(self, mock_makedirs, mock_file):
        ...
        with patch.object(
            self.reporter, "_generate_html_content"
    ...
    def test_generate_report_success_bulk_results(self, mock_makedirs, mock_file):
        ...
        with patch.object(
            ...
            result = self.reporter.generate_report(
                evaluation_results=mock_process_eval, output_path=self.output_path
            )
    ...
    def test_generate_report_invalid_evaluation_results(self):
        ...
        result = self.reporter.generate_report(
            evaluation_results=None, output_path=self.output_path
        )
    ...
    def test_generate_report_empty_output_path(self):
        ...
        result = self.reporter.generate_report(
            evaluation_results=individual_results, output_path=""
        )
    ...
    def test_generate_report_with_document_files(self, mock_makedirs, mock_file):
        ...
        individual_results = {"overall": {"cm_f1": 0.85}}
        document_files = {"doc1": "/path/to/doc1.pdf", "doc2": "/path/to/doc2.jpg"}

        with patch.object(
            self.reporter, "_copy_files_to_report_dir"
        ) as mock_copy_files:
    ...
    def test_copy_files_to_report_dir_success(self):
        ...
        document_files = {"doc1": source_file1, "doc2": source_file2}

        try:
            result = self.reporter._copy_files_to_report_dir(
                document_files, self.output_path
            )
    ...
    def test_build_html_document_basic(self):
        ...
        with patch.object(
            self.reporter, "_get_basic_css", return_value="body { margin: 0; }"
        ):
    ...
    def test_build_html_document_with_individual_docs(self):
        ...
        with patch.object(self.reporter, "_get_basic_css", return_value=""):
            with patch.object(
                self.reporter,
                "_get_javascript",
                return_value="<script>test();</script>",
            ):
                html = self.reporter._build_html_document(
                    sections, title, individual_docs, mock_schema
                )

        assert "<script>test();</script>" in html

    @patch(
        "stickler.reporting.html.utils.data_extractors.DataExtractor.extract_all_field_thresholds"
    )
    def test_get_javascript_with_data(self, mock_extract_thresholds):
        ...
        with patch.object(
            self.reporter, "_load_javascript_file", return_value="function test() {}"
        ):
