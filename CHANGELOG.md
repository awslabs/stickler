...
- `model_json_schema()` now describes the model's shape the way an equivalent
  ...
  Pass `from_json_schema(schema, tolerate_missing_fields=True)` to get the
  hand-written model's behavior instead; requiredness is preserved either way, so
  the field is still reported as required in a rendered schema. Tracked in
  [#214](https://github.com/awslabs/stickler/issues/214)
