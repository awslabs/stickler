"""Tests for structured model metrics calculation for nested structures with Invoice and LineItem models.
...
class TestStructuredObjectMetrics(unittest.TestCase):
    ...
    def test_confusion_matrix_aggregation_for_nested_objects(self):
        ...
        an Invoice, the confusion matrix contains proper entries for aggregated counts.
        """
        ...
        def get_base_metrics(cm_result, field_name):
            return {
                k: v
                for k, v in cm_result["fields"][field_name].items()
