...
- `non_match_collector.py`, `result_helper.py`,
  `evaluator_format_helper.py` — result shaping. Effectively-null checks for
  primitives/lists are inlined as private functions in `comparison_dispatcher.py`
  (their only caller) rather than a separate helper module.
