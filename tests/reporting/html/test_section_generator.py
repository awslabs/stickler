...
class TestSectionGenerator:
    ...
    def setup_method(self):
        ...
        self.section_generator = SectionGenerator(
            self.mock_results, self.mock_viz_engine
        )
    ...
    @patch(
        "stickler.reporting.html.utils.data_extractors.DataExtractor.extract_overall_metrics"
    )
    def test_generate_executive_summary(self, mock_extract_metrics):
        ...
        self.mock_viz_engine.generate_performance_gauge.return_value = (
            '<div class="gauge">85%</div>'
        )
        ...
        self.mock_viz_engine.generate_performance_gauge.assert_called_once_with(
            0.85, config
        )
    ...
    def test_generate_executive_summary_missing_document_count(
        self, mock_extract_metrics
    ):
    ...
    @patch(
        "stickler.reporting.html.utils.data_extractors.DataExtractor.extract_field_metrics"
    )
    def test_generate_field_analysis(self, mock_extract_field_metrics):
        ...
        self.mock_viz_engine.generate_field_performance_chart.return_value = (
            '<div class="chart">Chart</div>'
        )
        self.mock_viz_engine.generate_field_performance_table.return_value = (
            "<table>Table</table>"
        )
        ...
        self.mock_viz_engine.generate_field_performance_chart.assert_called_once_with(
            mock_field_metrics, config
        )
        self.mock_viz_engine.generate_field_performance_table.assert_called_once_with(
            mock_field_metrics, config
        )
    ...
    @patch(
        "stickler.reporting.html.utils.data_extractors.DataExtractor.extract_confusion_matrix"
    )
    def test_generate_confusion_matrix(self, mock_extract_cm):
        """Test confusion matrix generation."""
        mock_cm_data = {"tp": 45, "tn": 30, "fp": 5, "fn": 10}
        mock_extract_cm.return_value = mock_cm_data

        self.mock_viz_engine.generate_confusion_matrix_heatmap.return_value = (
        ...
        self.mock_viz_engine.generate_confusion_matrix_heatmap.assert_called_once_with(
            mock_cm_data, {}
        )
    ...
    @patch(
        "stickler.reporting.html.utils.data_extractors.DataExtractor.extract_non_matches"
    )
    ...
    def test_generate_document_gallery_image_mode(self):
        ...
        document_images = {"doc1": "images/doc1.jpg", "doc2": "images/doc2.png"}
    ...
    def test_generate_document_gallery_pdf_mode(self):
        ...
        document_pdfs = {"doc1": "pdfs/doc1.pdf", "doc2": "pdfs/doc2.pdf"}
        ...
        assert '<div class="document-gallery">' in result
        assert (
            '<div class="pdf-item" data-doc-id="doc1" data-pdf-path="pdfs/doc1.pdf">'
            in result
        )
        assert '<canvas id="pdf-canvas-doc1" class="pdf-canvas"></canvas>' in result
        assert (
            '<div class="pdf-loading" id="pdf-loading-doc1">Loading PDF...</div>'
            in result
        )
        assert (
            '<div class="pdf-error" id="pdf-error-doc1" style="display: none;">Error loading PDF</div>'
            in result
        )
        assert "<p><strong>doc1</strong></p>" in result
        assert "<p><strong>doc2</strong></p>" in result
...
class TestSectionGeneratorIntegration:
    ...
    def test_generate_sections_with_individual_results(self):
        ...
        individual_results = {
            "overall": {"f1": 0.82, "precision": 0.85, "recall": 0.79},
            "fields": {
                "name": {"f1": 0.90, "precision": 0.95, "recall": 0.85},
                "price": {"f1": 0.75, "precision": 0.80, "recall": 0.70},
            },
            "confusion_matrix": {"overall": {"tp": 20, "tn": 15, "fp": 3, "fn": 7}},
            "non_matches": [
                {
                    "field_path": "category",
