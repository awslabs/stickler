...
class LineItemModel(StructuredModel):
    ...
    air_date: str = ComparableField(
        ...
        threshold=0.9,
        description="Date the spot aired (MM/DD/YYYY)",
    )
    program: str = ComparableField(
        comparator=LevenshteinComparator(),
        threshold=0.7,
        description="Program or daypart name",
    )
    spot_length: str = ComparableField(
        comparator=LevenshteinComparator(),
        threshold=0.95,
        description="Spot length (e.g. :30)",
    )
    gross_rate: str = ComparableField(
        comparator=LevenshteinComparator(),
        threshold=0.9,
        description="Gross rate for this spot",
    )
    net_rate: str = ComparableField(
        comparator=LevenshteinComparator(),
        threshold=0.9,
        description="Net rate after commissions",
    )
...
class FccInvoiceModel(StructuredModel):
    ...
    station_name: str = ComparableField(
        comparator=LevenshteinComparator(),
        threshold=0.8,
        description="Broadcasting station name",
    )
    ...
    advertiser_name: str = ComparableField(
        comparator=LevenshteinComparator(),
        threshold=0.8,
        description="Political advertiser or candidate name",
    )
    bill_cycle: str = ComparableField(
        comparator=LevenshteinComparator(),
        threshold=0.9,
        description="Billing cycle (e.g. 02/16)",
    )
    gross_advertising_fee: str = ComparableField(
        comparator=LevenshteinComparator(),
        threshold=0.9,
        description="Gross advertising fee amount",
    )
    net_advertising_fee: str = ComparableField(
        comparator=LevenshteinComparator(),
        threshold=0.9,
        description="Net advertising fee after commissions",
    )
    total_this_invoice: str = ComparableField(
        comparator=LevenshteinComparator(),
        threshold=0.9,
        description="Total amount due on this invoice",
    )
    # Optional fields
    campaign_id: Optional[str] = ComparableField(
        ...
        comparator=LevenshteinComparator(),
        threshold=0.95,
        description="Campaign ID number",
    )
    estimate_id: Optional[str] = ComparableField(
        default=None,
        comparator=LevenshteinComparator(),
        threshold=0.95,
        description="Estimate ID number",
    )
    ...
    agency_commission: Optional[str] = ComparableField(
        ...
        comparator=LevenshteinComparator(),
        threshold=0.9,
        description="Agency commission amount",
    )
    rep_firm_commission: Optional[str] = ComparableField(
        default=None,
        comparator=LevenshteinComparator(),
        threshold=0.9,
        description="Rep firm commission amount",
    )
    line_items: List[LineItemModel] = ComparableField(
        default=None,
        comparator=LevenshteinComparator(),
        threshold=0.8,
        description="Individual spot/airtime line items",
    )
