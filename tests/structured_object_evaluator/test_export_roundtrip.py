...
def test_json_schema_roundtrip_comparison_behavior():
    ...
    assert (
        abs(result_orig["field_scores"]["name"] - result_recon["field_scores"]["name"])
        < 0.01
    )
    assert result_orig["field_scores"]["price"] == result_recon["field_scores"]["price"]
    assert (
        result_orig["field_scores"]["in_stock"]
        == result_recon["field_scores"]["in_stock"]
    )
...
def test_nested_model_roundtrip():
    ...
    # Create instances
    o1 = Order(
        order_id="ORD-001", product=Product(name="Laptop", price=999.99, in_stock=True)
    )
...
def test_list_model_roundtrip():
    ...
    ReconstructedProduct = ReconstructedCart.model_fields[
        "products"
...
def test_nested_model_custom_weight_json_schema_roundtrip():
    ...
    class Order(StructuredModel):
        order_id: str = ComparableField(threshold=1.0, default=...)
        product: Product = ComparableField(
            weight=3.0, clip_under_threshold=False, default=...
        )
    ...
    assert (
        schema["properties"]["product"].get("x-aws-stickler-clip-under-threshold")
        is False
    )
...
def test_list_model_custom_weight_json_schema_roundtrip():
    ...
    class Cart(StructuredModel):
        ...
        products: List[Product] = ComparableField(
            weight=5.0, clip_under_threshold=False, default=...
        )
    ...
    assert (
        schema["properties"]["products"].get("x-aws-stickler-clip-under-threshold")
        is False
    )
