...
!!! note "Rejected list elements are included in reports"

    A rejected pair contributes one `fd` in the confusion matrix and an indexed
    entry in `non_matches`. With `document_field_comparisons=True`, its row has
    `match=False` and `score=0.0`:

    ```
    [Plain(sku='a')]  vs  [Cat(sku='a')]     ->  fd=1,  non_matches: f[0]
    ```

    Both reports use the field's comparator and threshold for plain-model and
    primitive lists. Structured-model lists retain recursive matching and the
    element model's `match_threshold`.
