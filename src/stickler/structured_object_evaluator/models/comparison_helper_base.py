...
class ComparisonHelperBase(ABC):
    ...
    def generate_comparison_reason(
        self, is_match: bool, score: float, threshold: float
    ) -> str:
    ...
    def process_matched_pairs(
        ...
        for gt_idx, pred_idx, similarity_score in matched_pairs_with_scores:
            if gt_idx < len(gt_list) and pred_idx < len(pred_list):
                ...
                # Create reason
                reason = self.generate_comparison_reason(
    ...
    def process_unmatched_gt_items(
        self, field_name: str, gt_list: List[Any], assignments: List[tuple]
    ) -> List[Dict[str, Any]]:
    ...
    def process_unmatched_pred_items(
        self, field_name: str, pred_list: List[Any], assignments: List[tuple]
    ) -> List[Dict[str, Any]]:
    ...
    def collect_list_entries(
        ...
        entries.extend(
            self.process_unmatched_gt_items(field_name, gt_list, assignments)
        )

        # Process unmatched prediction items (FA)
        entries.extend(
            self.process_unmatched_pred_items(field_name, pred_list, assignments)
        )
