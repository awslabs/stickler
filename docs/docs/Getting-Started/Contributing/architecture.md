...
│       │   ├── (inline) standard result-dict factories
...
```python
# See: comparison_dispatcher.py:147-307
# Step 5 type routing:
...
```python
# See: comparison_dispatcher.py:123-145
@property
...
```python
# See: comparison_dispatcher.py:239-251
match (gt_effectively_null, pred_effectively_null):
    case (True, True):   return _create_true_negative_result(weight)
    case (True, False):  return _create_false_alarm_result(weight)
    case (False, True):  return _create_false_negative_result(weight)
...
```python
# See: comparison_dispatcher.py:259-266
